...
C. 删除外表只会删除 Inceptor 上的元数据不会删除数据文件，删除托管表两者都会 被删除
...
B. sqoop 抽取数据是个多节点并行抽取的过程，因此 map 的个数设置的越多性能 越好
...
B. 所有服务都启用 Kerberos 认证，用户要提供 Kerberos principal 和密码（或者keytab）来访问各个服务
...
A. 源数据文件存放于 hdfs 上，通过 load 命令加载数据文件，数据文件将被复制到 表目录下
...
D. 规划 HDFS 集群时，建议 Active NameNode 和 Standby NameNode 分配在不同的 机架上
...
B. add_global_index 't1', 'index_name','COMBINE_INDEX|INDEXED=f1:q1:9|rowKey:rowKey:10,UPDATE=true'
...
B. sqoop 抽取数据是个多节点并行抽取的过程，因此 map 的个数设置的越多性能越 好
...
A. SET ngmr.partition.automerge = TRUE;
...
C. Transwarp StreamSQL 可基于微批或事件对数据进行处理 D. 以上说法都不对
